...
import (
	...
	event "github.com/erniealice/cyta-golang/domain/event/event"
...
func buildProductsTable(
	...
	for _, p := range products {
		...
		unitPrice := fmt.Sprintf("%.2f %s", float64(p.GetUnitPrice())/100, p.GetCurrency())
		totalPrice := fmt.Sprintf("%.2f %s", float64(p.GetTotalPrice())/100, p.GetCurrency())
