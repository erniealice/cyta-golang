...
import (
	...
	event "github.com/erniealice/cyta-golang/domain/event/event_tag"
...
func eventTagColumns(l event.Labels) []types.TableColumn {
...
func buildTableRows(tags []*eventtagpb.EventTag, l event.Labels, common pyeza.CommonLabels, routes event.Routes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, t := range tags {
		active := t.GetActive()
		recordStatus := "active"
...
func statusPageTitle(l event.Labels, status string) string {
	switch status {
	case "active":
...
func statusPageCaption(l event.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l event.Labels, status string) string {
	switch status {
	case "inactive":
...
func statusEmptyMessage(l event.Labels, status string) string {
	switch status {
	case "inactive":
