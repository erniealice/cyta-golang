...
type Deps struct {
	Routes                  event.EventTagRoutes
	Labels                  event.EventTagLabels
...
func eventTagColumns(l event.EventTagLabels) []types.TableColumn {
...
func buildTableRows(tags []*eventtagpb.EventTag, l event.EventTagLabels, common pyeza.CommonLabels, routes event.EventTagRoutes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, t := range tags {
		active := t.GetActive()
		recordStatus := "active"
...
func statusPageTitle(l event.EventTagLabels, status string) string {
	switch status {
	case "active":
...
func statusPageCaption(l event.EventTagLabels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l event.EventTagLabels, status string) string {
	switch status {
	case "inactive":
...
func statusEmptyMessage(l event.EventTagLabels, status string) string {
	switch status {
	case "inactive":
