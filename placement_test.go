// placement_test.go — Wave-T placement gate for cyta (DOMAIN variant).
//
...
//
// cyta is a test-only package at root (root holds ZERO non-test .go files — R1
// is already satisfied), so this file uses `package cyta`; `go test` accepts a
// directory whose only .go files are _test.go. esqyma is found via the sibling
// ../esqyma/proto/v1/domain.
...
var crossCutting = false              // cyta is a domain package
var legacyAllow = map[string]string{} // EMPTY = STRICT (cyta is fully migrated)
