...
var crossCutting = false // cyta is a domain package (event domain).
// legacyAllow — each entry carries a dated EXPIRES stamp (no open-ended
// exemptions). Shrunk toward STRICT (empty) as far as the realized tree allows.
var legacyAllow = map[string]string{
	// domain/event/calendar/ is a domain-level calendar view package whose
	// directory is named "calendar" (not "eventcalendar"), so R2′ cannot
	// auto-recognise it as a domain-view (isDomainView requires the dir name to
	// start with the domain name "event"). Kept until the dir is renamed to
	// "eventcalendar" or the package is promoted into the event entity subdir.
	"domain/event/calendar": "domain-level calendar view, dir name does not start with 'event' — EXPIRES 2026-07-15 (capstone)",

	// domain/event/recurrence/ maps to the esqyma entity "event_recurrence" but
	// the directory is named "recurrence" (not "event_recurrence"), so entities["recurrence"]
	// is false and isDomainView fails (doesn't start with "event"). Kept until
	// the dir is renamed to "event_recurrence".
	"domain/event/recurrence": "esqyma entity is 'event_recurrence' but dir is 'recurrence' — EXPIRES 2026-07-15 (capstone)",
}
...
// (domain/<d>/<subcontext>/<entity>/). EMPTY for cyta — no sub-context layer.
